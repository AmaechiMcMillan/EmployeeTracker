...
async function main() {
  ...
  if (responseObject.option === "View All Departments") {
    ...
    const departments = [];
    const departmentList = await connection.execute(
    ...
    const departmentId = await connection.execute(
    ...
    const [rows] = await connection.execute(
      `INSERT INTO role (title, salary, department_id) 
        VALUES (?, ?, ?)`,
      [title, salary, departmentId[0][0].id]
    );
    ...
    const { first_name, last_name, role, manager_id } = await employeePrompt(
      roles
    );
    ...
    const [rows] = await connection.execute(
      `INSERT INTO employee (first_name, last_name, role_id, manager_id) 
        VALUES (?, ?, ?, ?)`,
      [first_name, last_name, roleId[0][0].id, manager_id]
    );
    console.log(
      "You have added " + first_name + " " + last_name + " to the employee list"
    );
    main();
  } else if (responseObject.option === "Update an Employee Role") {
    const names = [];
    const employeeNames = await connection.execute(
      "SELECT first_name, last_name FROM employee"
    );
    for (let i = 0; i < employeeNames[0].length; i++) {
      names.push(
        `${employeeNames[0][i].first_name} ${employeeNames[0][i].last_name}`
      );
    }
